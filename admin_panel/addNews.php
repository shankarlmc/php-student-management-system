...
if (isset($_POST['Update'])) { // if save button on the form is clicked
  $id = $_POST['id'];
    ...
    if(empty($errors)==true) {
        // move the uploaded (temporary) file to the specified destination
        move_uploaded_file($file, $destination);
        $sql ="UPDATE `news_table` SET `title`='$title',`content`='$content',`semester`='$semester',`file`='$filename',`value`='1' WHERE news_id = $id";
            if (mysqli_query($conn, $sql)) {
...
<div class="container">
  <div class="row">
    <?php if(isset($_GET['EditNews'])){
      $id = $_GET['id'];

      $sql = "SELECT * FROM `news_table` WHERE news_id = $id and value=1";
    ...
    <div class="col-md-7"> 
    <h1 class="font-weight-bold text-primary" style="">Wants to Edit News</h1><br>
    <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
                <div class="form-group">
                  ...
                  <input type="submit" value="Update News" name="Update" class="btn btn-success btn-lg btn-block">
                  <br>
            </div>

          <?php }else{ ?>
            <div class="col-md-7"> 
    <h1 class="font-weight-bold text-primary" style="">Wants to add News</h1><br>
    <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
                <div class="form-group">
                    <div class="form-check form-check-inline col-md-12">
                        <select class="custom-select none" name="semester"id="semester">
